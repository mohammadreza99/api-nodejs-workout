...
export class CellImageComponent implements ICellRendererAngularComp {
  ...
  onSingleRemove(event: Event) {
    ...
    this.utilsService
      .showConfirm(
        { header: 'حذف فایل', message: 'تصویر حذف شود؟' },
        this.vcRef
      )
      .then((res) => {
        if (res) {
    ...
    if (Array.isArray(value) && this.multiple) {
      ...
      for (const item of value) {
        ...
        if (item instanceof File) {
          ...
          if (wantBase64) resultToEmit.push(await this.fileToBase64(item));
          else if (wantFile) resultToEmit.push(item);
        }
        //Array of string
      ...
      if (wantBase64) this.filesToEmit.push(await this.fileToBase64(value));
      else if (wantFile) this.filesToEmit.push(value);
    } else if (typeof value == 'string') {
      //value is a single base64
      if (value.startsWith('src=')) {
        this.filesToShow.push(value);
        if (wantBase64) this.filesToEmit.push(value);
        else if (wantFile)
        ...
        if (wantBase64) this.filesToEmit.push(await this.urlToBase64(value));
        else if (wantFile) {
          this.filesToEmit.push(
            this.base64toFile(
